...
export class ExpenseProgress extends Component {
	render() {
		...
		return(
			<>
				<ProgressBar style={{fontSize: '1em', height: '3em', minWidth: '10em'}} variant="danger" now={expense_percent} label={`Expenses: $${expenses}`} />
				<ProgressBar style={{fontSize: '1em', height: '3em', minWidth: '10em'}} variant="success" now={revenue_percent} label={`Revenue: $${revenue}`}/>
			</>
		)
	}
}
